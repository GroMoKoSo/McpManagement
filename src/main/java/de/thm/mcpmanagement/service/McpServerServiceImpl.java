...
public class McpServerServiceImpl implements McpServerService {
    ...
    public GroMoKoSoMcpServer getServerForUser(@NonNull String username) {
        ...
        List<GetApiListResponseDto> apis = userManagementClient.getApisFromUser(username);
        var builder = serverProvider.builder().serverInfo("Server of %s".formatted(username), version);
        int n = 0;
        for (var api : apis) {
            ToolSet toolSet = toolSetRepository.findById(api.apiId()).orElse(null);
            if (toolSet == null) {
                logger.warn("Tool set with id {} not found, skipping", api.apiId());
                continue;
            }
